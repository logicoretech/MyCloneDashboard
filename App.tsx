...
const formatKpiCurrency = (value: number): string => {
  ...
  for (const item of suffixes) {
    if (abs >= item.value) {
      const shortened = value / item.value;
    ...
    <div className="min-h-screen bg-[#fcfcfd] pb-12">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
          <StatCard 
            label="Gross" 
            value={formatKpiCurrency(stats.totalPotentialRevenue)}
            icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
            colorClass="text-indigo-600"
          />
          <StatCard 
            label="Invoiced" 
            value={formatKpiCurrency(stats.totalInvoiceAmount)}
            icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>}
            colorClass="text-slate-600"
          />
          <StatCard 
            label="Collected" 
            value={formatKpiCurrency(stats.totalDollarsCollected)}
          ...
          <StatCard 
            label="Expenses" 
            value={formatKpiCurrency(stats.totalExpenseIncurred)}
          ...
          <StatCard 
            label="Net Profit" 
            value={formatKpiCurrency(stats.totalNetRevenue)}
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-[10px] text-gray-400 uppercase tracking-widest bg-white">
                <tr>
                  ...
                  <th className="px-6 py-4 text-right border-b border-gray-50">Gross</th>
